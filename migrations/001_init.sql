...
COMMENT ON TABLE skill_templates IS '技能模板表，存储标准化的Agent技能模板';
COMMENT ON COLUMN skill_templates.name IS '技能名称';
COMMENT ON COLUMN skill_templates.task_type IS '任务类型';
COMMENT ON COLUMN skill_templates.prompt_template IS '提示词模板';
COMMENT ON COLUMN skill_templates.tool_sequence IS '工具调用序列';
COMMENT ON COLUMN skill_templates.input_schema IS '输入参数Schema';
COMMENT ON COLUMN skill_templates.success_rate IS '成功率';
COMMENT ON COLUMN skill_templates.usage_count IS '使用次数';
...
-- ============================================
-- 表6: skill_evaluations - 技能评估表
-- ============================================
CREATE TABLE IF NOT EXISTS skill_evaluations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    skill_id UUID REFERENCES skill_templates(id) ON DELETE CASCADE,
...
COMMENT ON TABLE skill_evaluations IS '技能评估表，记录每次技能执行的质量评估';
COMMENT ON COLUMN skill_evaluations.skill_id IS '关联技能ID';
COMMENT ON COLUMN skill_evaluations.task_id IS '关联任务ID';
COMMENT ON COLUMN skill_evaluations.passed IS '是否通过';
COMMENT ON COLUMN skill_evaluations.score IS '评估分数';
COMMENT ON COLUMN skill_evaluations.feedback IS '评估反馈';

-- ============================================
-- 表7: long_term_memories - 长期记忆表
-- ============================================
...
COMMENT ON TABLE long_term_memories IS '长期记忆表，存储全局业务知识和历史经验';
COMMENT ON COLUMN long_term_memories.memory_type IS '记忆类型：session_insight/task_pattern/domain_knowledge';
COMMENT ON COLUMN long_term_memories.content IS '记忆内容';
COMMENT ON COLUMN long_term_memories.metadata IS '元数据';

-- ============================================
-- 索引：新增
-- ============================================
CREATE INDEX IF NOT EXISTS idx_skill_templates_task_type ON skill_templates(task_type);
